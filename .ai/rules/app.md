...
resources/views/layouts/app/sidebar.blade.php's Pets group now iterates `auth()->user()->currentShelter?->species()->orderBy('name')->get() ?? []` instead of `Species::query()->...->get()` — so the sidebar only shows species enabled for the logged-in manager/staff's currently active shelter (see [[models-models]] for `currentShelter()`/`current_shelter_id`). Don't revert to the global Species query; that would leak species across shelters and defeat the shelter_species setting.
