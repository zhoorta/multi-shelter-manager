...
## Viewer role is read-only and hides adopter/sponsor personal data
shelter_users.role now allows 'viewer' (manager/staff/viewer). Viewers can view pets, vaccinations and facilities, print (PetPrint/PetPrintList) and use the share-pet modal (it only holds public info, deliberately not gated by canEdit), and receive vaccination emails when their membership flag is on. Every Pets write path checks `abort_unless(Auth::user()->canEditCurrentShelter(), 403)`: form mounts (PetForm, AdoptionForm, SponsorshipForm, VaccinationForm), delete actions, and ManagesSponsorshipPayments methods. Views hide the controls with a single `@php $canEdit = auth()->user()->canEditCurrentShelter(); @endphp` at the top, so roleForShelter() isn't queried once per row. People's personal data (adopters, sponsors, volunteers, members) is hidden from viewers: ManageAdoptions/ManageSponsorships/AdoptionShow/SponsorshipShow use `abort_if(isViewerOfCurrentShelter(), 403)`, pet-show skips the adoption/sponsorship boxes, and the sidebar hides those links. ManageVolunteers/VolunteerShow and the members pages (see [[members]]) use the same abort_if, and the sidebar hides Volunteers and Members for viewers (decided 2026-09-25: a viewer sees animals and facilities, never people). Any new pets write action or personal-data screen must add the same checks.
