...
shelter_users.role now allows 'viewer' (manager/staff/viewer). Viewers can view pets, vaccinations, volunteers, facilities and print (PetPrint/PetPrintList), and receive vaccination emails when their membership flag is on. Every Pets write path checks `abort_unless(Auth::user()->canEditCurrentShelter(), 403)`: form mounts (PetForm, AdoptionForm, SponsorshipForm, VaccinationForm), delete actions, and ManagesSponsorshipPayments methods. Views hide the controls with a single `@php $canEdit = auth()->user()->canEditCurrentShelter(); @endphp` at the top, so roleForShelter() isn't queried once per row. Adopter/sponsor personal data is hidden from viewers: ManageAdoptions/ManageSponsorships/AdoptionShow/SponsorshipShow use `abort_if(isViewerOfCurrentShelter(), 403)`, pet-show skips the adoption/sponsorship boxes, and the sidebar hides those two links. Any new pets write action or personal-data screen must add the same checks.
