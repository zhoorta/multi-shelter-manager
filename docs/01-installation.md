...
`APP_LOCALE` sets the default interface language. Everyone sees it until they pick another language: users in **Settings → Appearance**, visitors in the language menu at the top of the public and login pages (see [Personal settings](15-settings.md#154-language)). Supported languages:
...
- `true` — the home page is a public website where visitors can browse the animals available for adoption in every shelter (see [chapter 13](14-public-portal.md)).
...
| `APP_LOCALE` | Default interface language, until a user or visitor picks another: `en`, `pt`, `es`, `fr`, `de`, `it`, `nl`, `pl`, `sv` or `da` | `en` | Your language |
