        ...
        <header class="sticky top-0 z-30 border-b border-amber-100 bg-amber-50/85 backdrop-blur dark:border-stone-800 dark:bg-stone-950/85">
            <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
                <a href="{{ route('home') }}" class="flex items-center gap-2" wire:navigate>
                    ...
                    <span class="font-[Fredoka] text-2xl font-semibold tracking-tight text-stone-900 dark:text-white">{{ config('app.name') }}</span>
                </a>

                <nav class="flex items-center gap-2">
                    <a href="#adopt" class="hidden rounded-full px-4 py-2 text-sm font-semibold text-stone-600 transition hover:bg-amber-100 hover:text-stone-900 sm:inline-block dark:text-stone-300 dark:hover:bg-stone-800 dark:hover:text-white">
                        {{ __('Adopt') }}
                    </a>
                    @auth
