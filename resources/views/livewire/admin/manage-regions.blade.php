<div class="flex h-full w-full flex-1 flex-col gap-6">
    <div class="flex items-center justify-between">
        <flux:heading size="xl">{{ __('Regions') }}</flux:heading>

        <flux:modal.trigger name="region-form">
            <flux:button variant="primary" icon="plus" wire:click="createRegion">
                {{ __('Create') }}
            </flux:button>
        </flux:modal.trigger>
    </div>
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                        @forelse ($this->regions as $region)
                            <tr wire:key="region-{{ $region->id }}">
                                <td class="px-6 py-3 font-medium text-neutral-900 dark:text-white">{{ $region->name }}</td>
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <flux:modal.trigger name="confirm-region-deletion-{{ $region->id }}">
                                            <flux:button
                                                size="sm"
                                                variant="subtle"
                                                icon="trash"
                                        ...
                                        <flux:modal name="confirm-region-deletion-{{ $region->id }}" class="max-w-lg">
                                            <div class="space-y-6">
                                                <div>
                                                ...
                                                <div class="flex justify-end space-x-2 rtl:space-x-reverse">
                                                    ...
                                                    <flux:button variant="danger" wire:click="deleteRegion({{ $region->id }})">
                                                        {{ __('Delete') }}
                                                    </flux:button>
                                                </div>
                                            </div>
    ...
    <flux:modal name="region-form" class="max-w-lg">
        <form wire:submit="saveRegion" class="flex flex-col gap-6">
            <flux:heading size="lg">
                {{ $editingRegionId ? __('Edit') : __('Create') }} &mdash; {{ __('Regions') }}
            </flux:heading>

            <flux:input wire:model="regionName" :label="__('Name')" />
            ...
            <div class="flex justify-end gap-2">
                ...
                <flux:button type="submit" variant="primary">
                    {{ $editingRegionId ? __('Save') : __('Create') }}
                </flux:button>
            </div>
        </form>
    </flux:modal>
