{{-- Public pet profile (photo slider, facts, description, shelter box), shared by the "public-pet-details" modal and the public pet page. Expects $pet (with species, breed, size, furType, shelter.region, images, cage.wing). Optional: $headingTag (h1 on the page), $showShelterLink, $isAdopted (hides the adoption and contact actions), $pageUrl (link to the full page, shown in the modal). --}}
@php
    ...
    $photoAlt = __(':name, :species :breed for adoption', ['name' => $pet->name, 'species' => Str::lower($pet->species->name), 'breed' => $pet->breed->name]);
@endphp
        ...
        <div x-ref="track" x-on:scroll.debounce.60ms="sync()" class="flex aspect-[4/3] snap-x snap-mandatory overflow-x-auto [scrollbar-width:none] sm:aspect-[16/10] [&::-webkit-scrollbar]:hidden">
            @foreach ($galleryUrls as $url)
                <div wire:key="public-pet-slide-{{ $loop->index }}" class="relative size-full shrink-0 snap-center overflow-hidden">
                    ...
                    <img src="{{ $url }}" alt="{{ $loop->first ? $photoAlt : $pet->name.' — '.__('Photo').' '.$loop->iteration }}" @if (! $loop->first) loading="lazy" @endif class="relative size-full object-contain">
                </div>
            @endforeach
        </div>
...
<div class="flex flex-col gap-5 p-6">
    <div class="flex items-start justify-between gap-4">
        <div>
            <{{ $headingTag }} class="font-display text-3xl font-bold text-stone-900 dark:text-white">{{ $pet->name }}</{{ $headingTag }}>
    ...
    <div class="flex flex-col gap-3 rounded-2xl bg-orange-50 p-4 dark:bg-orange-950/40">
        ...
        <div class="flex flex-wrap items-center justify-between gap-2">
            ...
            @if (filled($pageUrl ?? null))
                <a href="{{ $pageUrl }}" class="text-xs font-semibold text-orange-600 hover:underline dark:text-orange-300" wire:navigate>🔗 {{ __('Open this animal\'s page') }}</a>
            @endif
        </div>
    </div>
</div>
